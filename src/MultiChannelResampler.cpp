    ...
    float normalizedCutoff)
{
    ...
    const float cutoffScaler = normalizedCutoff * ((outputRate < inputRate) ? ((float)outputRate / inputRate) : ((float)inputRate / outputRate));
    const auto numTapsHalf = static_cast<float>(getNumTaps() / 2); // NOLINT // numTaps must be even.
    const float numTapsHalfInverse = 1.0f / numTapsHalf;
    for (int i = 0; i < numRows; i++) {
        float tapPhase = static_cast<float>(phase) - numTapsHalf;
        float gain = 0.0; // sum of raw coefficients
        int gainCursor = coefficientIndex;
        for (int tap = 0; tap < getNumTaps(); tap++) {
            auto radians = static_cast<float>(tapPhase * M_PI);

#if MCR_USE_KAISER
            float window = mKaiserWindow(tapPhase * numTapsHalfInverse);
#else
            auto window = static_cast<float>(mCoshWindow(tapPhase * numTapsHalfInverse));
#endif
        ...
        float gainCorrection = 1.f / gain; // normalize the gain
        for (int tap = 0; tap < getNumTaps(); tap++) {
